...
(function () {
  ...
  async function init() {
    try {
      ...
      const colsConfig = TableConfigModal.mergeConfig(
        "table_CQ_NhomLopHocPhan_Config",
        rawColsConfig,
      );
      ...
      myTable = new DataTable({
        ...
        isRowSelectable: (row) =>
          !(
            isTrueLike(row.XacNhan) ||
            Number(row.ID_LanTongHopFile || 0) > 0 ||
            isTrueLike(row.TrangThaiThanhToan)
          ),
        isRowEditable: (row) =>
          !(
            isTrueLike(row.XacNhan) ||
        ...
        getRowClass: (row) => {
          ...
          if (
            ...
            classes.push("row-confirmed");
          if (Number(row.ID_LanTongHopFile || 0) > 0)
            classes.push("row-signed");
          if (
            isTrueLike(row.ThanhToanThinhGiang) ||
            isTrueLike(row.TrangThaiThanhToan) ||
        ...
        customCellRender: (row, col) => {
          ...
          if (col.MaTruong === "LopChuyen") {
            ...
            if (isTrueLike(val)) {
              ...
              String(val).trim().toLowerCase() === "false" ||
              val === 0 ||
              val === "0"
            ) {
        ...
        onRowDirty: (row, key, val) => {
          ...
          if (key === "TenHTHoc") {
            const matched = hinhThucHocOptions?.find(
              (item) => item.text === val,
            );
            if (!matched) {
              ...
              if (typeof showToast !== "undefined")
                showToast("Vui lòng chọn hình thức học trong danh sách.", true);
              updateSaveButtonVisibility();
              return;
            }
          ...
          if (key === "TenHTDay") {
            const matched = hinhThucDayOptions?.find(
              (item) => item.text === val,
            );
            if (!matched) {
              ...
              row._dirty = Boolean(modifiedRows[rowId]);
              if (typeof showToast !== "undefined")
                showToast("Vui lòng chọn hình thức dạy trong danh sách.", true);
              updateSaveButtonVisibility();
              return;
            }
  ...
  async function loadTableData(hoc_ky) {
    try {
      ...
      const nhomLopData = await apiLopHocPhanChinhQuy.getNhomLopData(
        hoc_ky,
        filterParam,
      );
  ...
  // --- ĐIỀU HƯỚNG SPA & NAVBAR CONTEXT ---
  const contextReadyHandler = (e) => {
    const hoc_ky = e.detail;
    loadTableData(hoc_ky);
  };

  const contextChangedHandler = (e) => {
    const hoc_ky = e.detail;
    if (hasUnsavedChanges()) {
      if (typeof confirmModal !== "undefined") {
        confirmModal
          .show(
            "Có dữ liệu chưa lưu. Việc đổi học kỳ sẽ làm mất thay đổi. Tiếp tục?",
            "Cảnh báo",
            "Tiếp tục",
          )
          .then((isOk) => {
  ...
  // Đăng ký sự kiện
  window.addEventListener("ContextReady", contextReadyHandler);
  window.addEventListener("ContextChanged", contextChangedHandler);

  // Kích hoạt ngay lập tức cho SPA vì ContextReady chỉ chạy 1 lần lúc nạp trang đầu tiên
  if (typeof navbarState !== "undefined" && navbarState.selectedNamTaiChinh) {
    loadTableData(navbarState.selectedNamTaiChinh);
  }

  // Dọn dẹp sự kiện khi SPA chuyển trang
  window.pageCleanup = () => {
    window.removeEventListener("ContextReady", contextReadyHandler);
    window.removeEventListener("ContextChanged", contextChangedHandler);
  };
  ...
  function updateFooter(rows, selectedSet = null) {
    ...
    if (btnToggleBulkUpdate) {
      btnToggleBulkUpdate.style.display =
        selectedSize > 0 ? "inline-flex" : "none";

      // Nếu bảng không còn dòng nào được chọn nhưng panel vẫn đang mở -> tự động đóng lại
      if (
        selectedSize === 0 &&
        typeof isBulkPanelOpen !== "undefined" &&
        isBulkPanelOpen
      ) {
    ...
    if (statUnconfirmedEl)
      statUnconfirmedEl.textContent = formatNum(sumUnconfirmed);
  }
  ...
  function bindStaticEvents() {
    ...
    if (btnSave) {
      btnSave.addEventListener("click", async () => {
        ...
        if (selectedIds.length === 0) {
          if (typeof showToast !== "undefined")
            showToast(
              "Vui lòng chọn ít nhất một lớp học phần để xác nhận",
              true,
            );
          return;
        }
        ...
        try {
          ...
          const namTaiChinh =
            typeof navbarState !== "undefined"
              ? navbarState.selectedNamTaiChinh
              : null;

          const res = await apiLopHocPhanChinhQuy.confirmNhomLopHocPhan(
            selectedIds,
            namTaiChinh,
          );
          if (typeof showToast !== "undefined") {
            showToast(
              `Đã xác nhận thành công ${res.updated_count} lớp học phần.`,
            );
          }

          // Cập nhật giao diện cục bộ (Inline update) thay vì tải lại toàn bộ dữ liệu
          const updated_rows = selectedIds.map((id) => {
            const existingRow = myTable.state.data.find(
              (r) => String(r.MaNhomLopHP) === String(id),
            );
            return {
              ...existingRow,
              XacNhan: 1,
    ...
    if (btnToggleBulkUpdate && bulkUpdatePanel) {
      btnToggleBulkUpdate.addEventListener("click", async () => {
        ...
        btnToggleBulkUpdate.setAttribute(
          "aria-expanded",
          String(isBulkPanelOpen),
        );
        btnToggleBulkUpdate.textContent = isBulkPanelOpen
          ? "Đóng cập nhật hàng loạt"
          : "Cập nhật hàng loạt";
    ...
    if (btnSaveChanges) {
      btnSaveChanges.addEventListener("click", async () => {
        ...
        const targetRows = myTable
          .getRows()
          .filter(
            (row) =>
              !isTrueLike(row.XacNhan) &&
              selectedSet.has(String(row.MaNhomLopHP)),
          );
        ...
        if (hasBulkChange && affectedCount === 0) {
          if (typeof showToast !== "undefined")
            showToast(
              "Vui lòng chọn (tích vào ô vuông) ít nhất một bản ghi.",
              true,
            );
          return;
        }
    ...
    // Expose function for SPA Router
