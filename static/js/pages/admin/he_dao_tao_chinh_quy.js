(function () {
  ...
  function setLoadingState() {
    ...
    [
      funnelElements.rateXacNhan,
      funnelElements.rateKy,
      funnelElements.rateThanhToan,
    ].forEach((el) => {
      if (el) el.textContent = "--%";
    });
    [
      funnelElements.barXacNhan,
      funnelElements.barKy,
      funnelElements.barThanhToan,
    ].forEach((el) => {
      if (el) el.style.width = "0%";
    });
    if (funnelElements.chartCenterValue)
      funnelElements.chartCenterValue.textContent = "--%";
  }
  ...
  function renderDashboard(data) {
    if (elements.tongNhomLop)
      elements.tongNhomLop.textContent = formatNum(data.TongNhomLop);
    if (elements.daXacNhan)
      elements.daXacNhan.textContent = formatNum(data.DaXacNhan);
    ...
    if (elements.daThanhToan)
      elements.daThanhToan.textContent = formatNum(data.DaThanhToan);
    if (elements.chuaThanhToan)
      elements.chuaThanhToan.textContent = formatNum(data.ChuaThanhToan);
    if (elements.chuaXacNhan)
      elements.chuaXacNhan.textContent = formatNum(data.ChuaXacNhan);
  ...
  function renderFunnelRates(data) {
    ...
    if (funnelElements.rateXacNhan)
      funnelElements.rateXacNhan.textContent = formatPercent(rateXacNhan);
    if (funnelElements.rateKy)
      funnelElements.rateKy.textContent = formatPercent(rateKy);
    if (funnelElements.rateThanhToan)
      funnelElements.rateThanhToan.textContent = formatPercent(rateThanhToan);

    if (funnelElements.barXacNhan)
      funnelElements.barXacNhan.style.width = rateXacNhan + "%";
    if (funnelElements.barKy) funnelElements.barKy.style.width = rateKy + "%";
    if (funnelElements.barThanhToan)
      funnelElements.barThanhToan.style.width = rateThanhToan + "%";
  }
  ...
  const currentContext = sessionStorage.getItem("CTX_NAM_TAI_CHINH");
  if (currentContext) {
    loadDashboardStats(currentContext);
  }

  // Dọn dẹp sự kiện khi rời trang SPA
