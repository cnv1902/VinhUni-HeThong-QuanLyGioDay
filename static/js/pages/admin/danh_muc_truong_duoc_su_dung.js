  ...
  cbMaBang.setValue = async (value, triggerChange = true) => {
    ...
    if (triggerChange) {
      if (hasUnsavedChanges()) {
        if (!confirm("Bạn có thay đổi chưa lưu. Nếu chuyển đổi bảng, các thay đổi sẽ bị mất. Tiếp tục?")) {
...
async function loadTableData(maBang) {
  try {
    ...
    if (typeof showToast !== 'undefined') showToast("Không thể tải danh sách cấu hình!", true);
  }
}
...
async function initTable() {
  ...
  myTable = new DataTable({
    ...
    getCellEditorOptions: async (row, col) => {
      ...
      if (col.MaTruong === 'KieuTruong') {
        return ['text', 'number', 'select', 'badge', 'capacity', 'action', 'mono'];
      }
      // Các cột boolean: cung cấp select True/False thay vì datalist mặc định của badge
      if (col.MaTruong === 'HienThi' || col.MaTruong === 'DuocSua' || col.MaTruong === 'GhimCot') {
        return ['true', 'false'];
      }
      return null;
    },
  ...
  const colsConfig = TableConfigModal.mergeConfig('table_HETHONG_DMTruongDuocSuDung_Config', rawColsConfig);
...
async function initSaveEvents() {
  ...
  if (btnConfigTable && typeof TableConfigModal !== 'undefined') {
    btnConfigTable.addEventListener('click', () => {
      TableConfigModal.open('table_HETHONG_DMTruongDuocSuDung_Config', myTable.state.rawColumns, (newCols) => {
  ...
  if (btnSaveChanges) {
    btnSaveChanges.addEventListener('click', async () => {
      ...
      btnSaveChanges.innerHTML = '<span class="spinner" style="width:16px;height:16px;border-width:2px;"></span> Đang lưu...';

      try {
        const payload = { items: payloadItems };
        const data = await apiDanhMucTruongDuocSuDung.bulkUpdateDanhSachCot(payload);

        if (typeof showToast !== 'undefined') {
          showToast(`Lưu thành công ${data.updated_count || payloadItems.length} cấu hình cột!`);
        }
