class SemanticEditorDrawer {
  ...
  async open(groupId, groupName) {
    ...
    if (titleEl)
      titleEl.textContent = `Cấu hình Chi tiết: ${groupName || "Nhóm Công Thức"}`;
    ...
    const mappedHTD = this.hinhThucDayData.map((d) => ({
      id: d.MaHTDay,
      text: d.TenHTDay,
    }));
  ...
  async handleClose() {
    if (this.checkIsDirty()) {
      ...
      if (typeof confirmModal !== "undefined") {
        isConfirm = await confirmModal.show(
          "Cấu hình có thay đổi chưa được lưu. Bạn có chắc chắn đóng không?",
          "Đóng cấu hình",
        );
      }
      if (!isConfirm) return;
  ...
  async loadDrawerData(groupId) {
    try {
      ...
      const resHSDG = await apiSemanticEditor
        .getHeSoLopDongDonGian()
        .catch((e) => null);
      ...
      const resTH = await apiSemanticEditor
        .getTruongHopNhomCongThuc(groupId)
        .catch((e) => null);
      if (resTH && resTH.ok) {
  ...
  renderMenu() {
    ...
    this.DOM.menuDynamic.innerHTML = this.truongHopData
      .map((item) => {
        const isActive =
          String(this.activeMenuId) === String(item.id) ? "active" : "";
        return `
  ...
  async renderDetailView() {
    ...
    if (this.activeMenuId === "lop_dong") {
      ...
      const item = this.truongHopData.find(
        (x) => String(x.id) === String(this.activeMenuId),
      );
      if (item) {
  ...
  previewHTML(semanticText) {
    if (!semanticText || !semanticText.trim())
      return `<em>— Chưa có công thức —</em>`;
    ...
    escaped = escaped
      .split("LÀM_TRÒN")
      .join('<span class="tk tk-func">LÀM_TRÒN</span>')
      .split("NẾU")
      .join('<span class="tk tk-func">NẾU</span>');
    return escaped;
  }
  ...
  initEvents() {
    ...
    if (btnCancel)
      btnCancel.addEventListener("click", () => this.handleClose());
      ...
      .addEventListener("click", async (e) => {
        if (e.target.classList.contains("btn-delete-case")) {
          ...
          if (typeof caseId === "string" && !caseId.startsWith("th_")) {
            try {
              const res =
                await apiSemanticEditor.deleteTruongHopCongThuc(caseId);
              if (!res.ok) {
                const errorData = await res.json().catch(() => null);
                throw new Error(
                  errorData && errorData.detail
                    ? errorData.detail
                    : "Lỗi khi xóa từ server",
                );
              }
            } catch (err) {
          ...
          this.truongHopData = this.truongHopData.filter(
    ...
    document.getElementById("btnAddFormat").addEventListener("click", () => {
      ...
      if (!val) {
        if (typeof showToast !== "undefined")
          showToast("Vui lòng chọn hoặc nhập tên hình thức dạy!");
        return;
      }

      if (
        this.truongHopData.some(
          (x) => x.name.toLowerCase() === val.toLowerCase(),
        )
      ) {
        if (typeof showToast !== "undefined")
          showToast("Trường hợp này đã tồn tại!");
        return;
      }
    ...
    this.DOM.detailContent.addEventListener("input", (e) => {
      ...
      const rowEl =
        el.closest(".ld-grid-row") ||
        el.closest(".ld-card") ||
        el.closest(".hsld-group-block");
      ...
      if (el.classList.contains("dt-sync-group")) {
        ...
        const th = this.truongHopData.find(
          (x) => String(x.id) === String(this.activeMenuId),
        );
        if (th) {
    ...
    this.DOM.detailContent.parentNode.addEventListener("click", (e) => {
      ...
      if (
        e.target.closest(".btn-edit-row") ||
        e.target.closest(".btn-edit-expr")
      ) {
        const rowEl =
          e.target.closest(".ld-grid-row") ||
          e.target.closest(".expr-grid-row");
        if (rowEl) {
      ...
      // Thêm mốc hệ số lớp đông
        ...
        this.fullHeSoData[gIndex].rows.unshift({
          id: "ld_" + Date.now(),
          min: "",
          max: "",
          formula: "",
        ...
        this.renderDetailView().then(() => {
          const firstInput = this.DOM.detailContent.querySelector(
            `.ld-grid-row input[data-gindex="${gIndex}"][data-rindex="0"]`,
          );
      ...
      // Xóa nhóm hệ số lớp đông
        ...
        const gIndex = e.target.closest(".btn-delete-hsld-group").dataset
          .gindex;
        if (typeof confirmModal !== "undefined") {
          confirmModal
            .show("Bạn có chắc chắn muốn xóa mẫu này?", "Xóa Mẫu Cấu Hình")
            .then((isOk) => {
              if (isOk) {
      ...
      // Tạo nhóm hệ số lớp đông mới
        ...
        this.fullHeSoData.unshift({
          // Cho lên đầu danh sách để thấy ngay
          ID_HeSo_LD: null,
        ...
        this.renderDetailView().then(() => {
          const firstInput =
            this.DOM.detailContent.querySelector(".hsld-title-input");
          if (firstInput) {
            firstInput.focus();
      ...
      // Thêm phép tính trường hợp
        ...
        if (th) {
          ...
          this.renderDetailView().then(() => {
            ...
            const firstInput = this.DOM.detailContent.querySelector(
              `.expr-grid-row textarea[data-index="${newIndex}"]`,
            );
            if (firstInput) firstInput.focus();
          });
        }
    ...
    this.DOM.detailContent.addEventListener("focusout", (e) => {
      ...
      const rowEl =
        e.target.closest(".ld-grid-row") || e.target.closest(".expr-grid-row");
      if (rowEl) {
    ...
    if (this.DOM.masterToolbar) {
      this.DOM.masterToolbar.addEventListener("mousedown", (e) => {
        ...
        if (insertText) {
          ...
          this.activeEditor.dispatchEvent(
  ...
  async saveConfig() {
    ...
    btnSave.innerHTML =
      '<span class="semantic-spinner" aria-hidden="true"></span> Đang lưu...';
      ...
      if (this.activeMenuId === "lop_dong") {
        ...
        const payload = this.fullHeSoData.map((group) => {
          ...
          return {
            ...
            CauHinh_Json:
              validRows.length > 0 ? JSON.stringify(validRows) : null,
        ...
        if (!res.ok) {
          ...
          const errMsg =
            errorData && errorData.detail
              ? errorData.detail
              : "Lỗi khi lưu Hệ số lớp đông";
          throw new Error(errMsg);
        }

        if (typeof showToast !== "undefined")
          showToast("Đã lưu Hệ số lớp đông thành công!");
        ...
        const validTruongHop = this.truongHopData.map((t) => {
          const validExpressions = (t.expressions || []).filter(
            (e) => e.raw_formula && e.raw_formula.trim() !== "",
          );
          return {
            ID_TruongHop_CT:
              typeof t.id === "string" && t.id.startsWith("th_")
                ? null
                : parseInt(t.id),
            ...
            BieuThuc_JSON:
              validExpressions.length > 0
                ? JSON.stringify(validExpressions)
                : null,
            BieuThuc_Text: null,
            TrangThai: true,
          };
        ...
        const res = await apiSemanticEditor.bulkUpdateTruongHopNhomCongThuc(
          this.currentGroupId,
          validTruongHop,
        );
        ...
        if (!res.ok) {
          ...
          const errMsg =
            errorData && errorData.detail
              ? errorData.detail
              : "Lỗi khi lưu Trường hợp công thức";
          throw new Error(errMsg);
        }

        if (typeof showToast !== "undefined")
          showToast("Đã lưu danh sách Trường hợp công thức thành công!");
