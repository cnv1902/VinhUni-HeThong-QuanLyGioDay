...
function badgeHtml(v) {
  ...
  if (v === "Đã thanh toán")
    return `<span class="badge badge-blue">${iconLock()}${esc(v)}</span>`;
  if (v === "Đã xác nhận")
    return `<span class="badge badge-green">${esc(v)}</span>`;
...
class DataTable {
  ...
  constructor(config) {
    ...
    this.indexColumnStorageKey = this.storageKey
      ? `${this.storageKey}__DataTableMeta`
      : null;
  ...
  getRowId(row) {
    ...
    if (
      this.rowKey &&
      row[this.rowKey] !== undefined &&
      row[this.rowKey] !== null
    )
      return String(row[this.rowKey]);
    if (row.ID !== undefined && row.ID !== null) return String(row.ID);
    if (row.id !== undefined && row.id !== null) return String(row.id);
    if (row.MaNhomLopHP !== undefined && row.MaNhomLopHP !== null)
      return String(row.MaNhomLopHP);
  ...
  getRows() {
    ...
    if (this.state.sortKey) {
      ...
      const col = this.state.columns.find((c) => c.MaTruong === key);
      const isNum =
        col && (col.KieuTruong === "number" || col.KieuTruong === "capacity");
  ...
  cellDisplay(row, col) {
    ...
    switch (col.KieuTruong) {
      case "badge":
        return badgeHtml(v);
      case "badge_list":
        return this.badgeListHtml(v, col);
      case "capacity":
        return this.capacityHtml(row);
      case "action":
        return actionButtonsHtml(row);
      case "mono":
        return `<span class="mono-text">${esc(v)}</span>`;
      case "number":
        return `<span class="num-text">${this.formatNum(v, col.MaTruong)}</span>`;
      case "formula":
        return this.formatFormulaHtml(v);
      default:
  ...
  badgeListHtml(v, col) {
    ...
    const badges = items.map(
      (item) => `<span class="badge badge-gray">${esc(item)}</span>`,
    );
  ...
  formatFormulaHtml(v) {
    ...
    let html = String(v).replace(
      /\[VAR:([^\]]+)\]/g,
      '<span class="fm-var">$1</span>',
    );

    // 2. Format hàm (max, min, round)
    html = html.replace(
      /\b(max|min|round)\b/gi,
      '<span class="fm-func">$1</span>',
    );

    // 3. Format hằng số
    html = html.replace(
      /(?<!\[VAR:.*?)\b(\d+(\.\d+)?)\b/g,
      '<span class="fm-num">$1</span>',
    );
  ...
  capacityHtml(row) {
    ...
    if (ratio > 1) color = "var(--red-600)";
    else if (ratio >= 0.95) color = "var(--amber-600)";
   ...
   */
  buildHeaderHTML() {
    const selectableRows = this.getRows().filter((r) =>
      this.isRowSelectable(r),
    );
    const allSelected =
      selectableRows.length > 0 &&
      selectableRows.every((r) => this.state.selected.has(this.getRowId(r)));
    ...
    const indexResizeHandle = this.resizableColumns
      ? `<span class="col-resize-handle" data-key="__ROW_INDEX__" title="Kéo để đổi độ rộng cột"></span>`
      : "";
    ...
    const ths = this.state.columns
      .map((col) => {
        const stickyStyle = col.sticky
          ? `left:${this.stickyOffsets[col.MaTruong]}px;`
          : "";
        const resizeHandle = this.resizableColumns
          ? `<span class="col-resize-handle" data-key="${esc(col.MaTruong)}" title="Kéo để đổi độ rộng cột"></span>`
          : "";
  ...
  renderAll(options = {}) {
    ...
    const scrollTop =
      preserveScroll && this.container ? this.container.scrollTop : 0;
    ...
    if (totalRows === 0) {
      ...
      this.tbody.innerHTML = pageRows
        .map((r, index) => this.renderRowHTML(r, startIdx + index))
        .join("");
      if (this.incrementalRender && !this.enablePagination) {
    ...
    if (
      (totalRows === 0 || !this.tbody.querySelector(".incremental-trigger")) &&
      this.incrementalObserver
    ) {
      this.incrementalObserver.disconnect();
      this.incrementalObserver = null;
    }
    ...
    if (this.onRenderComplete)
      this.onRenderComplete(allRows, this.state.selected);
  ...
  renderIncrementalTrigger() {
    ...
    if (this.state.incrementalRenderedCount >= this.incrementalRowsCache.length)
      return;
  ...
  observeIncrementalTrigger() {
    ...
    this.incrementalObserver = new IntersectionObserver(
      (entries) => {
  ...
  appendNextIncrementalBatch() {
    ...
    const end = Math.min(
      start + this.incrementalBatchSize,
      this.incrementalRowsCache.length,
    );
    if (start >= end) return;
  ...
  async startEdit(td, rowId, colId) {
    ...
    if (this.isRowLocked(row)) {
      if (typeof showToast !== "undefined")
        showToast("Không thể sửa: bản ghi này đang bị khóa");
      return;
    }
    ...
    if (this.getCellEditorOptions) {
      try {
        ...
        if (typeof showToast !== "undefined")
          showToast("Không thể tải danh sách lựa chọn.", true);
      }
    }
    ...
    if (Array.isArray(externalOptions)) {
      ...
      const allOptions = Array.from(
        new Set(this.state.data.map((r) => String(r[col.MaTruong] || ""))),
      ).filter(Boolean);
      inputHtml =
        `<input type="text" class="cell-editor" list="${listId}" value="${esc(row[col.MaTruong])}">` +
    ...
    const commit = () => {
      ...
      if (
        (col.KieuTruong === "number" || col.KieuTruong === "capacity") &&
        val === ""
      ) {
        val = 0;
      }
    ...
    if (
      ...
      col.KieuTruong === "select" ||
      col.KieuTruong === "badge"
    ) {
  ...
  openFilterDropdown(key, btn) {
    ...
    div.style.left =
      Math.max(8, Math.min(rect.left, window.innerWidth - 256)) + "px";
    ...
    const allValues = Array.from(rawValuesMap.keys()).sort((a, b) =>
      rawValuesMap.get(a).localeCompare(rawValuesMap.get(b)),
    );
    let checkedSet = null;
    if (currentAllowed instanceof Set) checkedSet = currentAllowed;
    else if (currentAllowed && currentAllowed.values)
      checkedSet = currentAllowed.values;
    ...
    const mapItems = allValues
      .map(
        (v) =>
          `<label class="fd-item" data-text="${esc(rawValuesMap.get(v))}" style="display:flex; align-items:center; gap:8px;"><input type="checkbox" class="fd-cb" value="${esc(v)}" ${!checkedSet || checkedSet.has(v) ? "checked" : ""}>${getFilterLabel(v)}</label>`,
      )
      .join("");
    ...
    if (isNum) {
      ...
      let defaultMin =
        currentAllowed &&
        currentAllowed.min !== undefined &&
        currentAllowed.min !== null
          ? currentAllowed.min
          : "";
      let defaultMax =
        currentAllowed &&
        currentAllowed.max !== undefined &&
        currentAllowed.max !== null
          ? currentAllowed.max
          : "";
    ...
    div.querySelector(".fd-search").addEventListener("input", (e) => {
      ...
      div.querySelectorAll(".fd-item").forEach((lbl) => {
        const txt = (
          lbl.getAttribute("data-text") || lbl.textContent
        ).toLowerCase();
    ...
    div.querySelector(".fd-apply").addEventListener("click", () => {
      const checked = Array.from(div.querySelectorAll(".fd-cb:checked")).map(
        (cb) => cb.value,
      );
      let minVal = null,
        maxVal = null;
      ...
      if (
        checked.length === allValues.length &&
        minVal === null &&
        maxVal === null
      ) {
        delete this.state.filters[key];
  ...
  updateFilterCountBadge() {
    const n =
      Object.keys(this.state.filters).length + (this.state.search ? 1 : 0);
  ...
  startColumnResize(event, handle) {
    ...
    const col = isIndexColumn
      ? null
      : this.state.columns.find((c) => c.MaTruong === key);
    ...
    const startWidth = isIndexColumn
      ? this.indexColumnWidth
      : parseInt(col.DoRong, 10) || Math.round(rect.width);
  ...
  handleColumnResizeEnd() {
    ...
    if (state.isIndexColumn) {
      ...
      const rawCol = this.state.rawColumns.find(
        (c) => c.MaTruong === state.key,
      );
  ...
  persistColumnConfig() {
    ...
    const visibleMap = new Map(
      this.state.columns.map((col) => [col.MaTruong, col]),
    );
    let sourceColumns = this.state.rawColumns.length
      ? this.state.rawColumns
      : this.state.columns;
  ...
  loadIndexColumnWidth() {
    ...
    try {
      const savedMeta = JSON.parse(
        localStorage.getItem(this.indexColumnStorageKey) || "{}",
      );
  ...
  outsideClickHandler(e) {
    if (
      this.filterDropdownEl &&
      !this.filterDropdownEl.contains(e.target) &&
  ...
  bindEvents() {
    ...
    this.tbody.addEventListener("change", (e) => {
      ...
      if (cb.checked) this.state.selected.add(String(rowId));
      else this.state.selected.delete(String(rowId));
      ...
      if (selectAllCb) {
        ...
        selectAllCb.checked =
          ...
          selectableRows.every((r) =>
            this.state.selected.has(this.getRowId(r)),
          );
      }
    });
    ...
    this.thead.addEventListener("click", (e) => {
      ...
      if (sortBtn) {
        this.toggleSort(sortBtn.dataset.key);
        return;
      }
      const filterBtn = e.target.closest(".filter-btn");
      if (filterBtn) {
        this.openFilterDropdown(filterBtn.dataset.key, filterBtn);
        return;
      }
    });
