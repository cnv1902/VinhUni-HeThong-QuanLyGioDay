const apiCongThuc = {
  ...
  async getColumnsConfig() {
    try {
      const response = await fetch(
        `${window.API_PREFIX}/cau-hinh-chung/danh-sach-cot/HETHONG_NhomCongThuc`,
  ...
  async getCongThucData(id_he = null, trang_thai = null) {
    try {
      ...
      let hasFilter = false;
      if (id_he !== null && id_he !== "") {
        params.append("id_he", id_he);
        hasFilter = true;
      }
      if (trang_thai !== null && trang_thai !== "") {
        params.append("trang_thai", trang_thai);
        hasFilter = true;
      }
  ...
  async updateNhomCongThuc(id, payload) {
    try {
      const response = await fetch(
        `${window.API_PREFIX}/nhom-cong-thuc/${id}`,
        {
          method: "PUT",
          credentials: "same-origin",
          headers: {
  ...
  async getTuDienBienSo() {
    try {
      const response = await fetch(
        `${window.API_PREFIX}/cau-hinh-chung/tu-dien-bien-so`,
        { credentials: "same-origin" },
      );
      if (!response.ok) throw new Error("Network response was not ok");
      return await response.json();
  ...
  async getTuDienBienSo(id_he = 1, trang_thai = 1) {
    try {
      ...
      const response = await fetch(
        `${window.API_PREFIX}/cau-hinh-chung/tu-dien-bien-so?${params.toString()}`,
        { credentials: "same-origin" },
      );
