const apiDanhMucTruongDuocSuDung = {
  async getColumnsConfig() {
    try {
      const response = await fetch(
        `${window.API_PREFIX}/cau-hinh-chung/danh-sach-cot/HETHONG_DMTruongDuocSuDung`,
  ...
  async getDanhSachBang() {
    try {
      const response = await fetch(
        `${window.API_PREFIX}/cau-hinh-chung/danh-sach-bang`,
        { credentials: "same-origin" },
      );
  ...
  async getDanhSachCot(maBang) {
    try {
      const response = await fetch(
        `${window.API_PREFIX}/cau-hinh-chung/danh-sach-cot-admin/${maBang}`,
        { credentials: "same-origin" },
      );
      if (!response.ok)
        throw new Error(`Không thể lấy danh sách cột cho bảng ${maBang}`);
      return await response.json();
    } catch (error) {
  ...
  async bulkUpdateDanhSachCot(payload) {
    const response = await fetch(
      `${window.API_PREFIX}/cau-hinh-chung/danh-sach-cot/bulk-update`,
      {
        method: "PUT",
        credentials: "same-origin",
